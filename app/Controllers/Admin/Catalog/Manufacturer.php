...
namespace Carrito\Controllers\Admin\Catalog;
...
class Manufacturer extends Controller
{
    ...
    protected function getForm()
    {
        ...
        } elseif (!empty($manufacturer_info) && is_file($this->{'path.image'}.DIRECTORY_SEPARATOR.$manufacturer_info['image'])) {
    ...
    protected function validateForm()
    {
        ...
        if (utf8_strlen($this->request->post['keyword']) > 0) {
            $url_alias_info = $this->model_catalog_url->getUrlAlias($this->request->post['keyword']);
