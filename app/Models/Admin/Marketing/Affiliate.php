...
namespace Carrito\Models\Admin\Marketing;
...
class Affiliate extends Model
{
    ...
    public function editAffiliate($affiliate_id, $data)
    {
        ...
        if ($data['password']) {
            $this->db->query('UPDATE '.DB_PREFIX."affiliate SET salt = '".$this->db->escape($salt = random_str(9))."', password = '".$this->db->escape(sha1($salt.sha1($salt.sha1($data['password']))))."' WHERE affiliate_id = '".(int) $affiliate_id."'");
        }
